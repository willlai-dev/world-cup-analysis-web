...
} from '@/features/auth/pending-verification';
...
export function VerifyEmailPanel({ token }: { token?: string }) {
  ...
  // Handed over from login/register via sessionStorage — never via the URL,
  // so the address stays out of history/logs. null until read on the client.
  const [email, setEmail] = useState<string | undefined>(undefined);
  const [emailResolved, setEmailResolved] = useState(false);

  useEffect(() => {
    setEmail(readPendingVerificationEmail() ?? undefined);
    ...
    if (verifyMutation.isError) {
      ...
      return (
        <div className="flex flex-col gap-4">
          ...
          {emailResolved && <ResendSection initialEmail={email} />}
        </div>
      );
    }
